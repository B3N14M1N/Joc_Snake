...
Snake::Snake(sf::Texture& texture){
    ///Constructorul creaza un sarpe de lungimea 3, orientat in jos
...
void Snake::DrawSnake(sf::RenderTarget& window, int pixelsPerUnit){
    for(int i = 0; i < this->lenght; i++)
    {
        ...
        bodyPart.setScale(sf::Vector2f((float)pixelsPerUnit / (float)tilePixelSize, (float)pixelsPerUnit / (float)tilePixelSize));
        bodyPart.setPosition(sf::Vector2f(this->body[i].x * pixelsPerUnit, this->body[i].y * pixelsPerUnit));
        window.draw(bodyPart);
    }
};
bool Snake::IncreaseSnake(){
    ///Returneaza false daca lungimea a atins maximul admisibil
    if(this->lenght >= this->snakeMaxLenght)
        return false;
    else
    {
        ///Mareste lungimea sarpelui
        this->lenght++;
        
        ///Mareste dimensiunea sirurilor in care sunt salvate datele
        ///si sterge datele vehi
